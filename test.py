...
df_origin = pd.read_excel(docAddress,sheet_name='Report', header = None, names = ["产品","海关","当期出口金额（万美元）","当期进口金额（万美元）","当期出口数量（吨）","当期进口数量（吨）","一至当月出口金额（万美元）","一至当月进口金额（万美元）","一至当月出口数量（吨）","一至当月进口数量（吨）"], skiprows = 8)
'''
...
df = pd.DataFrame(columns = ["产品", "海关","海关地点","截至时间","时间","当期出口金额（万美元）","当期进口金额（万美元）","当期出口数量（吨）","当期进口数量（吨）","一至当月出口金额（万美元）","一至当月进口金额（万美元）","一至当月出口数量（吨）","一至当月进口数量（吨）"])
df['产品'] = df_origin['产品']
df['海关'] = df_origin['海关']
...
#删除海关地点中的“海关”、“关区”字样
#df_merge['海关地点'] = df_merge['海关地点'][:-2] 这个是删除了海关地点那列的最后两个数据
